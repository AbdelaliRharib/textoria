...
export default function AdminDashboard() {
  ...
  const exportGenerations = () => {
    try {
      ...
      const csvRows = generations.map(generation => [
        ...
        generation.type,
        generation.category,
        `"${generation.prompt.replace(/"/g, '""')}"`, // Escape quotes in prompt
        `"${generation.content.replace(/"/g, '""')}"`, // Escape quotes in content
      ...
      link.setAttribute('download', `generations-export-${new Date().toISOString().split('T')[0]}.csv`);
      ...
      toast.success('Generations exported successfully!');
    } catch (error) {
  ...
  const exportAnalytics = () => {
    try {
      ...
      const csvRows = [
        ['Total Users', analytics.totalUsers, 'Total number of registered users'],
        ['Active Users', analytics.activeUsers, 'Number of active users'],
        ['Total Generations', analytics.totalGenerations, 'Total content generations'],
        ['Text Generations', analytics.textGenerations, 'Number of text generations'],
        ['Image Generations', analytics.imageGenerations, 'Number of image generations'],
        ['Premium Users', analytics.premiumUsers, 'Number of premium subscribers'],
        ['Enterprise Users', analytics.enterpriseUsers, 'Number of enterprise subscribers'],
        ['Total Revenue', `$${analytics.totalRevenue}`, 'Total revenue generated'],
        ['Monthly Growth', `${analytics.monthlyGrowth}%`, 'Monthly growth percentage'],
        ['Average Generations Per User', analytics.averageGenerationsPerUser.toFixed(2), 'Average generations per user']
      ];
